...
def main():
    # 脚本所在目录（将 PDF 与本脚本放在同一文件夹即可直接运行）
    BASE_DIR = Path(__file__).parent

    pdf_path = BASE_DIR / "A case of portal vein recanalization and symptomatic heart failure.pdf"

    if not pdf_path.exists():
        print(f"❌ 找不到文件：{pdf_path}")
        sys.exit(1)

    entities = extract_all(str(pdf_path))

    output_path = BASE_DIR / "output-medical_entities.json"
